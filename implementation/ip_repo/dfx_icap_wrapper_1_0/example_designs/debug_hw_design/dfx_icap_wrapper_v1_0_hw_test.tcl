# Runtime Tcl commands to interact with - dfx_icap_wrapper_v1_0

# Sourcing design address info tcl
set bd_path [get_property DIRECTORY [current_project]]/[current_project].srcs/[current_fileset]/bd
source ${bd_path}/dfx_icap_wrapper_v1_0_include.tcl
...
# Test: s_axi_reg
# Create a write transaction at s_axi_reg_addr address
create_hw_axi_txn w_s_axi_reg_addr [get_hw_axis $jtag_axi_master] -type write -address $s_axi_reg_addr -data $wdata_1
# Create a read transaction at s_axi_reg_addr address
create_hw_axi_txn r_s_axi_reg_addr [get_hw_axis $jtag_axi_master] -type read -address $s_axi_reg_addr
# Initiate transactions
run_hw_axi r_s_axi_reg_addr
run_hw_axi w_s_axi_reg_addr
run_hw_axi r_s_axi_reg_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_s_axi_reg_addr]]
# Compare read data
if { $rdata_tmp == $wdata_1 } {
	puts "Data comparison test pass for - s_axi_reg"
} else {
	puts "Data comparison test fail for - s_axi_reg, expected-$wdata_1 actual-$rdata_tmp"
	inc ec
}
...
# Test: M_AXI_MEM
set wdata_m_axi_mem [get_init_data 0]
create_hw_axi_txn w_m_axi_mem_addr [get_hw_axis $jtag_axi_master] -type write -address $axi_gpio_out_addr -data ${wdata_m_axi_mem}
create_hw_axi_txn r_m_axi_mem_addr [get_hw_axis $jtag_axi_master] -type read -address $axi_gpio_in_addr 
# Initiate transactions
run_hw_axi r_m_axi_mem_addr
run_hw_axi w_m_axi_mem_addr
run_hw_axi r_m_axi_mem_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_m_axi_mem_addr]]
set DE [ get_done_and_error_bit $rdata_tmp 1 0 ]
# Compare read data
if { $DE == 01 } {
	puts "Data comparison test pass for - M_AXI_MEM"
} else {
	puts "Data comparison test fail for - M_AXI_MEM, rdata-$rdata_tmp expected-01 actual-$DE"
	inc ec
}
